...
#   TORQUE: raw torques sent directly to actuators.
#   PD: torques produced with PD control law.
#   CARTESIAN_PD: feet position converted in desired joint reference and successively PD converted in torques.

# Action space mode: DEFAULT, SYMMETRIC, SYMMETRIC_NO_HIP
#   DEFAULT: each joint controlled independently.
#   SYMMETRIC: right and left legs controlled symmetrically
#   SYMMETRIC_NO_HIP: as before, but hips forced to be 0.

# Observation space mode: look at 'sensor_collection.py' for more info.
# Recommended to follow the API for building your own sensors equipment.

# ENABLE_SPRINGS: if True, Parallel Elastic Actuators (PEA) are additionally simulated
