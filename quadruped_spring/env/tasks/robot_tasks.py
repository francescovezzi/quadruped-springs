...
class JumpingInPlaceDense(TaskJumping):
    ...
    def _reward(self):
        ...
        lin_vel = self._env.robot.GetBaseLinearVelocity()
        _, pitch, _ = self._env.robot.GetBaseOrientationRollPitchYaw()
        _, pitch_rate, _ = self._env.robot.GetTrueBaseRollPitchYawRate()
        pitch_reward = np.exp(-np.abs(pitch) / 0.1**2)
        pitch_rate_reward = np.exp(-np.abs(pitch_rate)**2 / 0.01)
        return 0
