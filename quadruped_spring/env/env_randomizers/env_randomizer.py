...
class EnvRandomizerMasses(EnvRandomizerBase):
    ...
    def _add_mass_offset(self):
        ...
        robot._add_base_mass_offset(base_mass,
                                    block_pos_delta_base_frame,
                                    is_render=self._env._is_render)
...
class EnvRandomizerDisturbance(EnvRandomizerBase):
    ...
    def __init__(self, 
                 env,
                 max_force = MAX_FORCE_DISTURBE,
                 max_time = MAX_TEMPORAL_DURATION,
                 time_range = DISTURBE_TIME_RANGE):
        self._env = env
    ...
    def _apply_disturbe(self):
        ...
        if time >= self._disturbe._temporal_istant and not self._timer.already_started():
            self._timer.start_timer(timer_time=time,
                                    start_time=time,
                                    delta_time=self._disturbe._duration)
...
class EnvRandomizerInitialConfiguration(EnvRandomizerBase):
    ...
    def _compute_new_init_config(self):
        ...
        sample_disturbe = np.random.uniform(np.zeros(self._env._robot_config.NUM_MOTORS),
                                            np.array(self._max_disturbe))
